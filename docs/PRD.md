...
**NODE (Network Of Domain Experts)** — 운영자 유광명을 허브로 한 36명(확장 가능)의 AI 도메인 전문가 네트워크를, 경력·소속·관심사 기반 관계로 추론하여 구형 지식그래프로 탐색하는 모던 정적 대시보드.
...
| 항목 | 요구사항 |
|---|---|
| 성능 | 36노드/추론 엣지 기준 인터랙션 시 60fps 목표. 초기 렌더(폴백 포함) 3초 이내. |
...
```
┌──────────────────────────────────────────────────────────┐
│ HEADER  ● NODE — Network Of Domain Experts   36 nodes  [Live] │
├───────────────┬──────────────────────────┬───────────────┤
...
- 현재 36명 기준으로 성능·레이아웃을 설계하되, 수백 명 규모는 Post-MVP에서 LOD/클러스터링으로 대응.
...
- [ ] **대규모 대응** — 수백 노드 시 LOD·클러스터링·엣지 번들링(MVP는 36명 규모)
