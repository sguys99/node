...
이 문서는 [docs/PRD.md](PRD.md)(v1.0)를 기반으로, **운영자 유광명을 중심 허브로 한 36명 AI 도메인 전문가 네트워크를 지식그래프로 시각화하는 제로빌드 정적 웹 대시보드**를 단계별로 구현하기 위한 작업 계획서입니다. 각 작업에는 진행 현황을 기록할 수 있는 체크박스(`- [ ]`)가 포함되어 있습니다.
...
### 2-1. 스냅샷 확보
- [x] PRD §3.1 시트 URL에서 CSV를 직접 fetch해 `data/snapshot.csv` 생성·커밋(36명 실데이터, 동일 스키마)
...
**검증:** 콘솔에서 정규화 결과 36건, 표준 태그 집합, 결측치 대체 로그 확인.
...
**검증(Playwright MCP):** 캔버스 렌더·노드 카운트 36·회전/줌 동작·노드 클릭 시 `selectedNodeId` 변화 확인, 초기 렌더 3초 이내.
...
### 6-1. 좌측 상세 패널
- [x] `js/panels.js` — 이름/닉네임/협업시점/나이/현직장/과거경력 표시(문서형 정보 리스트)
- [x] 하는일·관심사·희망사항을 표준 태그 칩으로 표시, `연결`(허브/소속/관심사 개수) 요약
- [x] 미선택 시 플레이스홀더("노드를 선택하면 상세 정보가 표시됩니다")

### 6-2. 우측 설정 패널
- [x] 라벨 표시 토글(이름/나이/닉네임/관심사) — 호버 툴팁 구성 제어(`render.js setLabelFields`)
- [x] 엣지 유형 토글(허브/소속/관심사 개별)
- [x] 회전 on/off(**기본 on**), 줌 리셋(카메라 초기 위치 복귀)
- [x] 설정 상태 런타임 메모리 유지(영속화는 Post-MVP)
...
### 7-1. 통합 검증
- [x] 수동 검증 체크리스트(아래 검증 지표 표 9항목) — 9/9 충족(인라인 hex 0건·5군집 소속 엣지·초기 렌더 1.7s 등)
- [x] Playwright E2E 시나리오: 로드 → 렌더 → 회전 → 노드 클릭 → 상세 → 토글 전체 플로우 — 통과(콘솔 에러 0건)
- [x] 접근성: 키보드로 줌/회전 리셋, 텍스트 대비 토큰 준수 — Tab 포커스·Space/Enter 동작, 전 텍스트 토큰 WCAG AA(≥6.19:1)
- [x] 반응형 최종 확인(데스크톱/태블릿/모바일) — 1440/800/375px 그래프 렌더 확인

### 7-2. 배포
- [ ] GitHub Pages 수동 설정: main 브랜치 루트 서빙(빌드 없음) — **운영자 수동 작업**(GitHub → Settings → Pages → Source: `main` / `/ (root)`)
- [ ] 배포 URL에서 정상 동작 확인 — Pages 설정 후 `https://sguys99.github.io/node/` 확인
...
| 항목 | 목표 |
|---|---|
| 데이터 로드 | 구글시트에서 전 구성원(현재 36명) 노드가 렌더링됨 |
