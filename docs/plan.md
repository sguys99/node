...
이 문서는 [docs/PRD.md](PRD.md)(v1.0)를 기반으로, **운영자 유광명을 중심 허브로 한 43명 AI 도메인 전문가 네트워크를 지식그래프로 시각화하는 제로빌드 정적 웹 대시보드**를 단계별로 구현하기 위한 작업 계획서입니다. 각 작업에는 진행 현황을 기록할 수 있는 체크박스(`- [ ]`)가 포함되어 있습니다.
...
### 2-1. 스냅샷 확보
- [x] PRD §3.1 시트 URL에서 CSV를 직접 fetch해 `data/snapshot.csv` 생성·커밋(44명 실데이터, 동일 스키마)
...
**검증(Playwright MCP):** 캔버스 렌더·노드 카운트 44·회전/줌 동작·노드 클릭 시 `selectedNodeId` 변화 확인, 초기 렌더 3초 이내.
...
| 항목 | 목표 |
|---|---|
| 데이터 로드 | 구글시트에서 전 구성원(현재 44명) 노드가 렌더링됨 |
